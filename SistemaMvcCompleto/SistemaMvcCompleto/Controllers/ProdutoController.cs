...
namespace SistemaMvcCompleto.Controllers
{
    ...
    public class ProdutoController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? Pesquisa, string? Categoria)
        {
            ...
            // Corrigido: usa '!' para verificar se a lista NÃO tem elementos
...
[HttpPost]
        ...
        // 📅 TELA VISUAL DE VENCIMENTOS (Retorna a View)
        [Authorize]
        [Authorize(Roles = "Admin")]
        ...
        // Substitua todas as chamadas para _produtoService.GetAllProdutosAsync() sem argumentos
        // por chamadas que passem explicitamente os parâmetros 'pesquisa' e 'categoria'.
        // Exemplo de correção para métodos afetados:

        //public async Task<IActionResult> EstoqueBaixo()
        //{

        //    // Corrigido: fornecendo argumentos nulos explicitamente
        //    var todosProdutos = await _produtoService.GetAllProdutosAsync(null, null);

        //    var produtosBaixos = todosProdutos
        ...
        {
            // 1. Busca os produtos filtrados por Pesquisa e Categoria
            var produtos = await _produtoService.GetAllProdutosAsync(Pesquisa, Categoria);

            // 2. Filtra na memória os que estão com estoque baixo
            ...
            // 3. Verifica se o resultado final de produtos com estoque baixo veio vazio
        ...
        // 📊 EXPORTAR VENCIMENTOS PARA EXCEL (Gera o arquivo)
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> ExportarVencimentos(string pesquisa, string categoria, string statusVencimento)
        {
            ...
            foreach (var p in listaVencimentos)
            {
                ...
                // Proteção contra data inválida ou não informada (MinValue)
                if (p.Validade != DateTime.MinValue && p.Validade.Year >= 1900)
                {
                    worksheet.Cell(linha, 5).Value = p.Validade;
                    worksheet.Cell(linha, 5).Style.DateFormat.Format = "dd/MM/yyyy";
        ...
        //// 📉 Consulta Visual de Estoque Baixo (≤ 10 unidades)
        //public async Task<IActionResult> EstoqueBaixo()
        //{
        //    // Buscando através do serviço para manter o padrão arquitetural
        //    var todosProdutos = await _produtoService.GetAllProdutosAsync(null, null);

        //    // Filtra os itens com quantidade menor ou igual a 10
        //    var produtosBaixos = todosProdutos
        //        .Where(p => p.QuantidadeEstoque <= 10)
        //        .OrderBy(p => p.QuantidadeEstoque)
        //        .ToList();
